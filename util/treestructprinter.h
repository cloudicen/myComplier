...
private:
    struct backLog
    {
        QWeakPointer<const treeNode> node;
        int nextIndex=0;
        backLog(const QWeakPointer<treeNode> &_node=QWeakPointer<treeNode>(),int _nextIndex=0):node(_node),nextIndex(_nextIndex){};
    };
...
public:
    static void printTree(const QWeakPointer<const treeNode> root)
    {
        int level=0;
        QWeakPointer<const treeNode> node=root;
        backLog nbl;
        ...
        while (true)
        {
            if (!node.isNull()) {
                ...
                if (node.toStrongRef()->isLeaf() || sub_index == 0) {
                    nbl.node = QWeakPointer<treeNode>();
                ...
                if (sub_index == branchCount-1) {
                    ...
                    printf("%s\n",node.toStrongRef()->toPrintable().toStdString().c_str());
                }
                node = node.toStrongRef()->children[sub_index];
            } else {
